...
export async function GET(request: Request) {
    try {
        ...
        if (!(await hasPageAccess('/visitoranalytics')) && !(await hasPageAccess('/visitoradmin'))) {
        ...
        const uniqueRecent = recentRes.rows.map(row => ({
            ...
            status: row.status,
            time: new Date(row.time).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })
        }));
