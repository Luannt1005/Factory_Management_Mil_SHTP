...
export async function GET(request: Request) {
    try {
        ...
        // Date Filter logic:
        // 1. If specific 'date' is provided, show visits active on that date (startDate <= date <= endDate)
        // 2. If 'startDate' and 'endDate' range is provided, show visits active within that range
        if (date) {
            whereConditions.push(`r."startDate" <= $${paramCount}::date AND r."endDate" >= $${paramCount}::date`);
            queryParams.push(date);
            paramCount += 1;
        } else if (startDate && endDate) {
            whereConditions.push(`r."startDate" <= $${paramCount+1}::date AND r."endDate" >= $${paramCount}::date`);
            queryParams.push(startDate, endDate);
            paramCount += 2;
        }

        if (requestCode) {
            whereConditions.push(`r.id ILIKE $${paramCount}`);
        ...
        if (search) {
            whereConditions.push(`(v.elem->>'name' ILIKE $${paramCount} OR v.elem->>'company' ILIKE $${paramCount} OR r.id || '-V' || v.idx ILIKE $${paramCount})`);
        ...
        const mainQuery = `
            ...
            LEFT JOIN "VisitorCheckInOut" c ON r.id = c."requestId" AND (v.idx::int - 1) = c."visitorIndex"
            ${whereClause}
            ORDER BY r."createdAt" DESC, v.idx ASC
        ...
        const statsQuery = `
            ...
            LEFT JOIN "VisitorCheckInOut" c ON r.id = c."requestId" AND (v.idx::int - 1) = c."visitorIndex"
            WHERE r."startDate" <= $1::date AND r."endDate" >= $1::date
        `;
